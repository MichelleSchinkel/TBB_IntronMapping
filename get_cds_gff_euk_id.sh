...
for id in $ids; do
    ...
    origid=$(grep -m 1 $id ~julian/julian2/snel-clan-genomes/eukarya/proteomes_metadata/$abbr.metadata.txt | cut -f 11)
    echo $origid
    if [[ $origid = "ENS"* ]]; then
	origid=${origid%.*} ## HSAP, CINT, DRER, XTRO, APLA, PSIN, OANA: Ensembl IDs --> exclude decimal at end
    fi
    if ! [[ -f ~julian/julian2/snel-clan-genomes/eukarya/gff/$abbr.gff3.gz || -f ~julian/julian2/snel-clan-genomes/eukarya/gff/$abbr.gff.gz ]]; then
	...
	if [[ $abbr = "ODIO" ]]; then ## ODIO: GSOIDP# in metadata file --> GSOID_T# in gff --> ID=gene# in same line --> grep rna# lines
	    ...
	    origid=rna$(zcat ~julian/julian2/snel-clan-genomes/eukarya/gff/ODIO.gff.gz | grep $origid | sed -r 's/.+ID=gene//; s/;Name.+//')
	    pattern="$origid;"
	elif [[ $abbr = "BSCH" ]]; then
	...
	zcat ~julian/julian2/snel-clan-genomes/eukarya/gff/$abbr.gff*.gz | grep $pattern | grep -w CDS | cut -f 1,3,4,5,7 | sed -r "s/^.+CDS/$id/" >> $output
    fi
done
