...
def usage():
    print("\n\npath_to_script.py [-h] [options] <supergroup_info.tsv> <cds_info.tsv> <alignments.fa>\n")
    print("<supergroup_info.tsv> must be a table with Orthologous gene, corresponding eukarya-4-identifier.")
    print("<cds_info.tsv> must be a table with eukarya-4-identifier, start, stop, direction.")
    print("<alignments.fa> must be an alignment with header >OG_eukarya-4-identifier.\n")
    print("Options:")
    print("-o: output directory (default: current directory)")
    print("-s: amount of shifts to tolerate (default: 2)")
    print("-g: percentage of genes in which intron has to be present (default: 7.5)")
    print("-p: percentage of species in which intron has to be present (default: 15)")
    print("-u: intron has to be present in at least X unikont and bikont species (default: 2)\n")
    
optlist, args = getopt.getopt(sys.argv[1:], 'o:s:g:p:u')

if len(args) != 3:
    print("Please provide a supergroup file, cds information and alignments")
    usage() ; sys.exit()
...
# Amount of shifts to tolerate 
if '-s' in opts.keys():
    shifts = opts['-s']
else:
    shifts = 2

# Intron has to be present in at least this many genes
if '-g' in opts.keys():
    threshold_percentage_genes = opts['-g']
else:
    threshold_percentage_genes = 7.5
    
# Intron has to be present in at least this percentage of all the species
if '-p' in opts.keys():
    threshold_percentage_species = opts['-p']
else:
    threshold_percentage_species = 15
    
# Intron has to be present in at least X Opimoda (~unikont) and X Diphoda (~bikont) species
if '-u' in opts.keys():
    threshold_species = opts['-u']
else:
    threshold_species = 2

species_to_supergroups = sys.argv[1]   
information_about_CDS = sys.argv[2]
your_protein_alignment = sys.argv[3]
    ...
    for coding_sequence in euk_cds_dict[key]:
        if direction == 'none':
            ...
            if coding_sequence[0] != direction:
                print ("ERROR: the CDSs of ",key," are not in the same direction")
...
for line in align:
    ...
    if line.startswith(">"): 
        ...
        if species not in supergroups:
            print("Error: ",line[end+1:end+5]," not in supergroup file", file = sys.stderr)
        else:
